...
$for_hire   = "No";               // Change to "No" if you are closed
...
// 3. SEO & META SETTINGS (For Google & Discord Embeds)
...
// 4. TECH STACK (Add as many as you want)
// Format: "Name" => ["years" => Number, "desc" => "Description"]
$tools = [
...
// ---------------------------------------------------------
// INSTRUCTIONS: 
// To DISABLE a link, add // at the start of the line.
// To ENABLE a link, remove the // from the start.
// ---------------------------------------------------------
...
// ==========================================
//         END CORE LOGIC (Do not touch anything above the line!)
// ==========================================
?>
    ...
    <nav class="fixed top-0 w-full z-50 py-4 md:py-5 px-6 glass">
        <div class="max-w-6xl mx-auto flex justify-between items-center">
            <div class="flex items-center gap-2 md:gap-3">
                <span class="font-black text-xl md:text-2xl italic text-blue-500 tracking-tighter uppercase leading-none"><?php echo $name; ?></span>
            ...
            <div class="hidden md:flex space-x-8 text-xs font-black uppercase tracking-[0.2em]">
                ...
                <a href="#work" class="hover:text-blue-400 transition">Experience</a>
                <a href="#tools" class="hover:text-blue-400 transition">Stack</a>
                <a href="#contact" class="hover:text-blue-400 transition">Contact</a>
            </div>
            <button id="menu-btn" class="md:hidden text-blue-500 p-2 focus:outline-none">
        ...
        <div id="mobile-menu" class="hidden absolute top-full left-0 w-full glass border-t border-white/5 py-6 px-6 flex flex-col space-y-4 text-center font-black uppercase tracking-widest italic">
            <a href="#" onclick="toggleMenu()" class="text-sm hover:text-blue-400 py-2">Home</a>
            <a href="#work" onclick="toggleMenu()" class="text-sm hover:text-blue-400 py-2 border-t border-white/5">Experience</a>
            <a href="#tools" onclick="toggleMenu()" class="text-sm hover:text-blue-400 py-2 border-t border-white/5">Stack</a>
            <a href="#contact" onclick="toggleMenu()" class="text-sm hover:text-blue-400 py-2 border-t border-white/5 border-b">Contact</a>
        </div>
    </nav>

    <div class="pt-40 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
        <section id="home" class="text-center">
            <div class="relative inline-block mb-10 animate-bounce-slow">
                <img src="<?php echo $avatar_url; ?>" class="w-56 h-56 rounded-full border-4 border-blue-600 shadow-[0_0_50px_rgba(37,99,235,0.3)]">
            </div>
            ...
            <div class="flex flex-wrap justify-center gap-4">
                <a href="#work" class="bg-blue-600 hover:bg-blue-700 text-white px-10 py-4 rounded-xl font-black uppercase italic transition-all shadow-xl">Portfolio</a>
        ...
        <section id="work">
            <div class="flex items-center gap-4 mb-12">
                <div class="h-10 w-2 bg-blue-500"></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                ?>
                <div class="glass rounded-[2rem] overflow-hidden group border border-white/5 transition-all hover:border-blue-500/30 flex flex-col">
                    ...
                    <div class="p-8 flex flex-col flex-grow">
                        ...
                        <div class="mt-auto">
                            <a href="https://www.roblox.com/games/<?php echo $rootPlaceId; ?>/" target="_blank" class="inline-flex items-center gap-2 text-blue-400 font-black text-xs tracking-widest uppercase hover:text-white transition group">
        ...
        <section id="tools">
            <div class="flex flex-col items-center mb-12 text-center">
                <h2 class="text-5xl font-black italic uppercase tracking-tighter mb-2 text-blue-500">Tech Stack</h2>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <?php foreach($tools as $toolName => $info): ?>
                    <div onclick="showToolDetail('<?php echo $toolName; ?>', '<?php echo $info['years']; ?>', '<?php echo addslashes($info['desc']); ?>')" 
                         class="glass p-6 rounded-2xl border border-white/5 uppercase font-black italic tracking-widest text-sm transition-all hover:scale-105 hover:border-blue-500/50 hover:text-blue-400 group cursor-pointer">
                        <span class="relative">
                            <?php echo $toolName; ?>
        ...
        <section id="contact">
            <h2 class="text-5xl font-black mb-12 italic uppercase text-center tracking-tighter">Let's <span class="text-blue-500">Connect</span></h2>
            <div class="max-w-xl mx-auto grid gap-4">
                <?php foreach($links as $label => $url): ?>
                    <a href="<?php echo $url; ?>" target="_blank" class="glass p-6 rounded-2xl flex justify-between items-center group hover:bg-blue-600/20 hover:border-blue-500/50 transition-all duration-500 relative overflow-hidden">
                        ...
                        <div class="flex flex-col text-left">
                            <span class="text-[10px] text-blue-500 font-bold uppercase tracking-[0.3em] mb-1">Direct Link</span>
                            <span class="font-black italic uppercase tracking-widest text-xl"><?php echo $label; ?></span>
                        </div>
                        <span class="text-2xl group-hover:translate-x-2 transition-all duration-300 text-blue-500">→</span>
                    </a>
                <?php endforeach; ?>
            </div>
        </section>
...
<footer class="glass mt-20 border-t border-white/5 py-10 text-center">
  <div class="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-6 md:flex-row">
    <div class="text-left">
      <p class="font-black text-blue-500 uppercase italic"><?php echo $name; ?></p>
    ...
    <div class="flex flex-col gap-2 md:items-end">
      <div class="flex gap-4 text-[10px] font-black tracking-widest uppercase">
        <a href="#" class="transition hover:text-blue-400">Back to top</a>
        <span class="text-gray-800">|</span>
        <span class="text-gray-500">
          Template by
          <a href="https://www.roblox.com/users/319189457/profile" target="_blank" rel="noopener noreferrer" class="text-white transition hover:text-blue-400"> UN6107617 </a>
        </span>
      </div>
      ...
      <div class="flex gap-3 text-[9px] font-bold tracking-tighter text-gray-600 uppercase">
        ...
        <span>•</span>
        <a href="https://devforum.roblox.com/u/un6107617/summary" target="_blank" class="hover:text-blue-500">Roblox DevForum</a>
      </div>
    </div>
  </div>
</footer>
    ...
    <div id="toolModal" class="fixed inset-0 z-[100] flex items-center justify-center px-6 hidden">
        ...
        <div class="glass max-w-lg w-full p-10 rounded-[2.5rem] border-blue-500/30 relative z-10 text-center scale-90 transition-transform duration-300" id="modalContent">
            <h3 id="modalTitle" class="text-5xl font-black italic uppercase tracking-tighter text-blue-500 mb-2"></h3>
            <div class="inline-block bg-blue-600/20 text-blue-400 border border-blue-500/30 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest mb-6"><span id="modalYears"></span> Years Experience</div>
